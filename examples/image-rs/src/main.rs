// TODO: keeping this since we'll soon add full support for PNG
...
fn main() {
    ...
    let input_path = match args.next() {
        Some(path) => path,
        None => {
            eprintln!("Please specify the input file path (must be a jpeg or webp)");
            exit(1);
        }
    };
    ...
    let output_path = match args.next() {
        Some(path) => path,
        None => {
            eprintln!("Please specify the output file path (must have a .jpg extension)");
            exit(1);
        }
    };
